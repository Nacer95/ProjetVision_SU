...
// ---------------------------------------
{
    ...
    X8 = ui8matrix(0-b, h-1+b, 0-b, w8-1+b);
    ...
    zero_ui8matrix(X8,  0-b, h-1+b, 0-b, w8    );
    ...
    //full_1_ui8matrix(X,  0-b, h-1+b, 0-b, w1-1+b);
    //puts("display");
    ...
    //puts("done\n");
    // display_ui8matrix (X,   0, h-1, 0, w0-1, "%5d", "X0");
    // display_ui8matrix (X8,  0, h-1, 0, w8-1, "%5d", "X8");

    // display_ui8matrix(Y_bas,         0, h-1, 0, w0-1, "%2d", "Y bas        ");
...
// ---------------------------------------
{
    ...
    min3_ui8matrix_ilu3_elu2_red       (X, 0, h-1, 0, w0-1, Y_ilu3_elu2r ); // puts("ilu3_elu2_red_factor");
    min3_ui8matrix_ilu3_elu2_red_factor(X, 0, h-1, 0, w0-1, Y_ilu3_elu2rf); // puts("ilu3_elu2_red_factor");


    //puts("done\n");
    //display_ui8matrix(X,               0, h-1, 0, w0-1, "%5d", "X            ");
    ...
    //display_ui8matrix(Y_elu2r,       0, h-1, 0, w0-1, "%5d", "Y elu2r      ");
    //display_ui8matrix(Y_elu2rf,      0, h-1, 0, w0-1, "%5d", "Y elu2rf     ");
    //display_ui8matrix(Y_ilu3_elu2r,      0, h-1, 0, w0-1, "%5d", "Y elu2rf     ");
    //display_ui8matrix(Y_ilu3_elu2rf,   0, h-1, 0, w0-1, "%5d", "Y ilu3 elu2rf");
...
// ----------------------------------------------
{
    ...
    // ouverture3_ui8matrix_pipeline_ilu3_red            (X, 0, h-1, 0, w0-1, T_pipeline_ilu3_red            , Y_pipeline_ilu3_red            );///P
    ouverture3_ui8matrix_pipeline_elu2_red            (X, 0, h-1, 0, w0-1, T_pipeline_elu2_red            , Y_pipeline_elu2_red            );
    // ouverture3_ui8matrix_pipeline_elu2_red_factor     (X, 0, h-1, 0, w0-1, T_pipeline_elu2_red_factor     , Y_pipeline_elu2_red_factor     );///P
    ouverture3_ui8matrix_pipeline_ilu3_elu2_red       (X, 0, h-1, 0, w0-1, T_pipeline_ilu3_elu2_red       , Y_pipeline_ilu3_elu2_red       );
    // ouverture3_ui8matrix_pipeline_ilu3_elu2_red_factor(X, 0, h-1, 0, w0-1, T_pipeline_ilu3_elu2_red_factor, Y_pipeline_ilu3_elu2_red_factor);///P
...
// -----------------------
void bench_morpho_min(int n0, int n1, int nstep)
// -----------------------
{
...
// ----------------------
{
    ...
    // min3_ui8matrix_basic(X, 0, h-1, 0, w-1, E);
    // max3_ui8matrix_basic(E, 0, h-1, 0, w-1, D);
    //
    // max3_ui8matrix_basic(D, 0, h-1, 0, w-1, O);
    // min3_ui8matrix_basic(O, 0, h-1, 0, w-1, F);
...
// ---------------------------------------------------
{
    ...
    for(int n = n0; n <= n1; n += nstep) {
        ...
        // BENCH(max3_ui8matrix_basic                               (X, 0, h-1, 0, w0-1,          Y_basic),  n ,    cpp_basic      );


        BENCH(min3_ui8matrix_basic               (X, 0, h-1, 0, w0-1, Y_bas        ), n, cpp_max_basic);
        BENCH(min3_ui8matrix_reg                 (X, 0, h-1, 0, w0-1, Y_reg        ), n, cpp_max_reg);
        BENCH(min3_ui8matrix_rot                 (X, 0, h-1, 0, w0-1, Y_rot        ), n, cpp_max_rot);
        BENCH(min3_ui8matrix_red                 (X, 0, h-1, 0, w0-1, Y_red        ), n, cpp_max_red);
        BENCH(min3_ui8matrix_ilu3                (X, 0, h-1, 0, w0-1, Y_ilu3       ), n, cpp_max_ilu3);
        BENCH(min3_ui8matrix_ilu3_red            (X, 0, h-1, 0, w0-1, Y_ilu3r      ), n, cpp_max_ilu3_red);
        BENCH(min3_ui8matrix_elu2_red            (X, 0, h-1, 0, w0-1, Y_elu2r      ), n, cpp_max_elu2_red);
        BENCH(min3_ui8matrix_elu2_red_factor     (X, 0, h-1, 0, w0-1, Y_elu2rf     ), n, cpp_max_elu2_red_factor);
        BENCH(min3_ui8matrix_ilu3_elu2_red       (X, 0, h-1, 0, w0-1, Y_ilu3_elu2r ), n, cpp_max_ilu3_elu2_red);
        BENCH(min3_ui8matrix_ilu3_elu2_red_factor(X, 0, h-1, 0, w0-1, Y_ilu3_elu2rf), n, cpp_max_ilu3_elu2_red_factor);
        // MAX ===========================================================================================
        ...
        char * nomfichier = "bench_resultats/test1.txt";
        char * line_de_commande = "";
        FILE * inputFile;
        inputFile = fopen( nomfichier, "a" );
        if ( inputFile == NULL ) {
            fprintf( stderr, "Cannot open file \n" );
            exit( 0 );
        }
        fprintf( inputFile, "%d %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f\n", n,
        cpp_max_basic, cpp_max_reg, cpp_max_rot, cpp_max_red, cpp_max_ilu3_red, cpp_max_elu2_red,
        cpp_max_elu2_red_factor, cpp_max_ilu3_elu2_red, cpp_max_ilu3_elu2_red_factor);

        fclose( inputFile );

        fprintf(line_de_commande, "python3 graphique.py %s", nomfichier);

    }

    system(line_de_commande);
